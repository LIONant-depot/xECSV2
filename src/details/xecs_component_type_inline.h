namespace xecs::component::type::details
{
    ...
    { return std::is_same_v<xecs::component::entity, T_COMPONENT>
                // NOTE(port): was type::guid{ nullptr } under xcore::guid::unit, which had a nullptr_t
                // constructor for "invalid guid". xresource::guid<T_ARG> has no such constructor (only
                // default, from-string-literal, and from-std::uint64_t), so this uses default-construction,
                // which yields the same zero m_Value.
                ? type::guid{}
    ...
    namespace details
    {
        ...
        template< typename T_COMPONENT >                   struct has_full_serialize< T_COMPONENT, std::void_t<decltype(&T_COMPONENT::FullSerialize)> > : std::true_type
        {
            ...
            __inline constexpr static xerr FullGenericSerialize(xecs::serializer::stream& TextFile, bool isRead, std::byte* pComponentArray, int& Count) noexcept
            {
        ...
        //---------------------------------------------------------------------------------
        // Content hash for a component's raw bytes, seeded with its type guid. Old xECS used
        // xcore::crc<64>{}.FromBytes(bytes, Seed) here; xresource_guid has no byte-span hash (only
        // a null-terminated-string one, guid<T>::GenerateGUIDCopy(const char*, seed), unsuitable for
        // POD bytes that may contain embedded zeros), so this reuses the exact same MurmurHash3-64
        // mixing xresource::guid_generator uses internally, adapted to a length-known byte span.
        constexpr
        std::uint64_t HashBytes( std::span<const std::byte> Data, std::uint64_t Seed ) noexcept
        {
        ...
        template< typename T_COMPONENT, typename = void >  struct has_share_compute_key : std::false_type
        {
            __inline constexpr static xecs::component::type::share::key Function(const std::byte* pComponent) noexcept
            {
                ...
                return { HashBytes({ pComponent, sizeof(T_COMPONENT) }, Guid.m_Value) };
            }
        };
    ...
    // NOTE(port): xcore::property::isValidTable<T>() -> xproperty::meta::reflected_object_type<T>
    // (a concept, not a function template - see xproperty.h). xcore::property::getTableByType<T>()
    // -> NOT xproperty::getObjectByType<T>(): that reads a pointer that XPROPERTY_REG's dynamic
    // initialization only sets at runtime (a global's constructor), unusable during consteval - and
    // this whole function needs to stay consteval, since CreateInfo() (below) builds a compile-time
    // xecs::component::type::info that embeds the resolved table directly, and references_mode_v's
    // AUTO-detection branch (above) needs to scan it during constant evaluation too. Instead this goes
    // through decltype(T::PropertiesDefinition())::get() - decltype never evaluates the call, just
    // deduces the xproperty::def<Name,T,Args...> it returns, whose ::register_v is itself an
    // `inline static constexpr` object (populated by consteval meta_t::getInfo()) and whose ::get() is
    // itself consteval - so this reaches the exact same table data without touching the runtime
    // registration at all.
    template< typename T_COMPONENT >
    consteval const xproperty::type::object* getPropertyTable(void) noexcept
    {
        if constexpr (xproperty::meta::reflected_object_type<T_COMPONENT>)
        {
            using reflected_t = xproperty::settings::reflected_type_t<T_COMPONENT>;
            return decltype(reflected_t::PropertiesDefinition())::get();
        }
        else
        {
            return nullptr;
    ...
    //---------------------------------------------------------------------------------
    // Recursively scans a property scope for any member whose underlying atomic type is
    // xecs::component::entity (a plain `var`, e.g. "parent", or a `list_var`, e.g. "children" -
    // xecs::component::entity is registered as a real xproperty atomic type by
    // xecs_entity_xproperty_bridge.h, so its guid_v is comparable against a member's
    // m_AtomicType.m_GUID). Used by references_mode_v's AUTO-detection below to decide between
    // BY_PROPERTIES and NO_REFERENCES the same way the old xcore::property data_variant-index scan
    // did, just against xproperty's member model instead of a finite variant of C++ types.
    //
    // Only var/list_var members and nested compile-time `scope` members (xproperty::type::members::scope
    // - a value-embedded compound with no per-instance dependency, e.g. xproperty::type::scope<...>)
    // can be inspected this way at compile time. `props`/`list_props` (pointer-based/polymorphic nested
    // objects) resolve their nested xproperty::type::object through m_pCast(void* pClassInstance, ...),
    // which needs a live instance we don't have here, so they're conservatively skipped - a component
    // whose only entity references live inside such a nested object still needs an explicit
    // typedef_v.m_ReferenceMode instead of AUTO.
    consteval bool ScopeHasEntityReferenceProperty( const xproperty::type::members::scope& Scope ) noexcept
    {
        for( auto& Member : Scope.m_Members )
        {
            if( auto* pVar = std::get_if<xproperty::type::members::var>(&Member.m_Variant) )
            {
                if( pVar->m_AtomicType.m_GUID == xproperty::settings::var_type<xecs::component::entity>::guid_v )
                    return true;
            }
            else if( auto* pListVar = std::get_if<xproperty::type::members::list_var>(&Member.m_Variant) )
            {
                if( pListVar->m_AtomicType.m_GUID == xproperty::settings::var_type<xecs::component::entity>::guid_v )
                    return true;
            }
            else if( auto* pScope = std::get_if<xproperty::type::members::scope>(&Member.m_Variant) )
            {
                if( ScopeHasEntityReferenceProperty(*pScope) )
                    return true;
            }
        }
        return false;
    ...
    constexpr auto references_mode_v = []() consteval noexcept -> type::reference_mode
    {
        ...
        else
        {
            ...
            else
            {
                ...
                else if constexpr (xproperty::meta::reflected_object_type<T_COMPONENT> == false) return type::reference_mode::NO_REFERENCES;
                else
                {
                    // AUTO-detect: does this component have any property whose atomic type is
                    // xecs::component::entity (see ScopeHasEntityReferenceProperty above)? If so it needs
                    // BY_PROPERTIES resolution, otherwise it has no entity references at all.
                    if( ScopeHasEntityReferenceProperty(*getPropertyTable<T_COMPONENT>()) )
                        return type::reference_mode::BY_PROPERTIES;
                    else
    ...
    type::info CreateInfo( void ) noexcept
    {
        ...
        ,   .m_Size                 = xecs::types::static_cast_safe<std::uint16_t>(sizeof(T_COMPONENT))
    ...
    requires(xecs::types::is_specialized_v<std::tuple, T_TUPLE>)
    using sort_tuple_t = xecs::types::tuple_sort_t<smaller_component, T_TUPLE>;
    ...
    }(xecs::types::null_tuple_v< sort_tuple_t<T_TUPLE> >);

    template< typename...T_TUPLES_OR_COMPONENTS >
    using combined_t = xecs::types::tuple_cat_t
    <   std::conditional_t
        <
            xecs::types::is_specialized_v<std::tuple, T_TUPLES_OR_COMPONENTS>
